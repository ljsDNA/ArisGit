...
{
    return _currentMappingID.startsWith("XFUNC") || _currentMappingID.startsWith("CFF");
}
...
function processSubShapesOfArisObject(p_visio, p_visShape, p_visShapeToProcess)
{
    ...
    function handleBPMNEvent(arisSymbolType, p_visio, p_visShape, p_visShapeToProcess)
    {
        ...
        {
            var sTriggerValue = getFirstMatchingPropValue(p_visShapeToProcess.vShape, ["StartEventTrigger", "BpmnTriggerOrResult_Start"], p_visio)//e.g. in ML RSPA Tech Proc V2 sample:
        ...
        {
            var sTriggerValue = getFirstMatchingPropValue(p_visShapeToProcess.vShape, ["EndEventResult", "BpmnTriggerOrResult_End"], p_visio)//e.g. in ML MS Visio KS sample:
        ...
        else if (arisSymbolType==Constants.ST_BPMN_INTERMEDIATE_EVENT)
        {
            ...
            var sTriggerValue = getFirstMatchingPropValue(p_visShapeToProcess.vShape, ["IntermediateEventTrigger", "BpmnTriggerOrResult_Intermediate"], p_visio)//e.g. in ML RSPA Tech Proc V2 sample:
    ...
    {
        var sExplicitTaskTypeValue = getFirstMatchingPropValue(p_visShapeToProcess.vShape, ["GatewayType", "BpmnGatewayType"], p_visio)//e.g. in ML Pizza sample
        var nDefTaskSymbol = null
    ...
    {
        var sExplicitTaskTypeValue = getFirstMatchingPropValue(p_visShapeToProcess.vShape, ["TaskType", "BpmnTaskType"], p_visio)//e.g. in ML Pizza sample
        var nDefTaskSymbol = null
    ...
    function getFirstMatchingPropValue(vShape, sPropNameU, p_visio)
    {
        var sValue = p_visio.getLoader().getFirstMatchingShapeProperty(vShape, sPropNameU)
        return sValue.toLowerCase();
    }
...
function postProcessCreatedObjects(p_tVisio, p_listAllCreatedShapes)
{
    ...
    while(itShape.hasNext() )
    {
        ...
        var mapShapeUserProperties = createdShape.shapeToProcess.oldProperties; //here visio stores the connection between object and swimlane *sic
