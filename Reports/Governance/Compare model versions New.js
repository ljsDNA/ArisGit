...
Context.setProperty("excel-formulas-allowed", false); //default value is provided by server setting (tenant-specific): "abs.report.excel-formulas-allowed"
 ...
 // Globals-----------------------------------------------------------------------------------------
	...
	var ga_OpenDBs 		= new java.util.HashSet();
...
// MAIN part---------------------------------------------------------------------------------------
    ...
    try {
		...
		// ensure to close all opened db connections
        var iterator = ga_OpenDBs.iterator();
...
function getModelByProps(sPropGuid, sPropRev) {
    ...
    var oDB = ArisData.getActiveDatabase();
    if (nRev > 0) {
        oDB = ArisData.openDatabaseVersion(oDB.Name(gn_Lang), nRev);
        ga_OpenDBs.add(oDB);
    }
...
function tryLoginToDB( oUsrSettings, p_bReadOnly ){
    if (!gb_showDlg100) {
        ...
        ga_OpenDBs.add(oUsrSettings.CompDB);
...
function getCurrentModelVersion( oDB, currModel) {  // AGA-4461
    ...
    if (maxVersionID > 0) {
        ...
        ga_OpenDBs.add(specifiedRevisionDB);
        ...
        switch( oDifference.getDifferenceType() ){// Get Existence OR Difference
            ...
            case Constants.MODCOMP_DIFF_BOTH:// Difference
                if( oDifference.isSubElementChanged() ){// Get Difference in Attribute 
                    ...
                    if( oAttr != null && oAttr.IsValid() &&((oAttr.KindNum == Constants.CID_ATTRDEF) || (oAttr.KindNum == Constants.CID_ATTROCC))){
        ...
        switch( oDifference.getDifferenceType() ){// Get Existence OR Difference
            ...
            case Constants.MODCOMP_DIFF_BOTH:// Difference
                if( oDifference.isSubElementChanged() ){//  Write Attribute difference message input
                    ...
                    var oAttr   = getRightLeftSubElement( null, oDifference );
                    if( oAttr != null && oAttr.IsValid() && ((oAttr.KindNum == Constants.CID_ATTRDEF) || (oAttr.KindNum == Constants.CID_ATTROCC)) && isAttributeToIgnore( getRightLeftSubElement( null, oDifference ) ) == false ){
