...
const COL_SAG_GREY = getColorByRGB(242, 242, 242);
...
function writeAttributeCellWithTemplate(uiValue, style, icon, template) {
    if (icon) {
        var totalColspan = style.colspan;
        style.colspan = 1;
        var image = icon.getImage();
        tableCellF("", style, template);
        g_ooutfile.OutGraphic(Context.createPicture(image, Constants.IMAGE_FORMAT_PNG), -1, 1, 1);      
        style.colspan = totalColspan - 1;
        tableCellF(uiValue, style, template);
    } else {
        tableCellF(uiValue, style, template);
    }
}
...
function getGroupsOfAttributesToDisplay(object) {
    ...
    for (var i = 0; i < metadataGroups.size(); i++) {
        ...
        var group = {"attributes" : attributes, "name": arcmGroup.getGroupName(), "totalAttr": totalAttr, "level": arcmGroup.getGroupLevel(), "isTopLevel" : arcmGroup.isTopLevelGroup(), "groupId" : arcmGroup.getGroupId()};        
...
function printHeadersRowWithTemplate(object, attributeIds, template){
     g_ooutfile.TableRow();  
     var baseColspan = 2; // for icon and uiValue
     for (var i = 0; i < attributeIds.length; i++) {
        var attributeUiName = getAttributeTypeName(object, attributeIds[i]);
        var style = {'colspan': baseColspan};
        writeAttributeCellWithTemplate(attributeUiName, style, null, template);
    }
}
...
function writeAttributeRowInListWithTemplate(attrRow, attributes, rowsForObj, template) {
    var baseColspan = 2; // for icon and uiValue
    g_ooutfile.TableRow();
    for (var i = 0; i < attributes.length; i++) {
        var valueRow = attributes[i].valueRows[attrRow];
        if (valueRow) {
            var col = valueRow.valueCols[0];
            var rowspan = rowsForObj / attributes[i].valueRows.length;
            var style = {'colspan':baseColspan, 'rowspan':rowspan};
            writeAttributeCellWithTemplate(getShortenedValue(col.uiValue), style, col.icon, template);
        }
    }
}
...
function getAttributeTypeName(object, attributeType) {
    if (object.isValueAttributeType(attributeType)) {
        ...
        return object.getCompositeAttribute(attributeType).getName();
    }
}
...
function tableCellF(text, style, template){
    style = getStyle(style);
    if(isColspanTable){
        g_ooutfile.TableCellF(text, style.rowspan, style.colspan, template);
    } else {
        g_ooutfile.TableCellF(text, style.width, template); 
    }
}
...
/*
    ARCM 'Confirmation owner' privilege check
*/
...
/*
    ARCM 'Confirmation auditor' privilege check
*/
function isConfirmationAuditor() {
    var user = umcComponent.getCurrentUser();
    return findPrivilegesForUser(user, "CFM_AUDITOR", true, false) != null;  
}

/*
    ARCM 'Confirmation manager' privilege check
*/
...
/*
    'ARCM administrator' privilege check
*/
function isArcmAdmin() {
    var user = umcComponent.getCurrentUser();
    return findPrivilegesForUser(user, "ARCM_ADMIN", true, false) != null;  
}


/*
    Get specific privilege of user 
*/
...
/*
    Get display name of the specificprivilege
*/
...
function getShortenedValue(textValue){
     if(textValue instanceof String && textValue.length() > 150){
        textValue = textValue.substring(0, 150);
        textValue = textValue + "...";
     }
     return textValue;
}
